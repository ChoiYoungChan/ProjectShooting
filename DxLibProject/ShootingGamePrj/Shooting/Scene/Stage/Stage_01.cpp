...
namespace game
{
	...
	void Stage01::SpawnMonster()
	{
		...
		{
			if(monster_move[count].isActive == false)
			{
				continue;
			}
	...
	void Stage01::CalkTask()
	{
		for (int index = 0; index < MONSTER_COUNT; index++)
		{
			if (enemy[index].monster_pos_x > 0 && enemy[index].monster_pos_x < WINDOW_SIZE_X - 50 &&
 				enemy[index].monster_pos_y > 0 && enemy[index].monster_pos_y < WINDOW_SIZE_Y - 50)
			{
		...
		{
			//printfDx("wwwwwwwwwwwwwwwwwwwww U LOSE!!");
		}
	}
	/*
	/// <summary>
